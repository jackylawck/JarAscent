/**
 * js/rocket_game.js - JarAscent 3D 任務主控 (雙語 + 簡易/進階模式)
 * @license MIT
 */
...
let rocket = null;
let currentLang = 'zh';
let isAdvancedMode = false; // 🟢 預設為簡易模式
...
export const CAM_MODE = { LAUNCH_PAD: 0, LIFTOFF: 1, ASCEND: 2, MAX_Q: 3, STAGE_SEP: 4, ORBIT: 5 };
...
const I18N = {
    zh: {
        ...
        configTitle: "🛠️ 任務與發射場設定", lblEnv: "發射場天色:", lblEngine: "火箭型號:", lblPayload: "任務載荷艙:",
        lblFuel: "燃料加注量 (%):", lblThrottle: "節流閥 (%):", lblTurn: "轉向高度 (km):",
        ...
        gaugeQ: "動態氣壓 (Max-Q 極限: 55 kPa)", gaugeFuel: "推進劑餘量",
        ...
        payloadOptions: { "8000": "新一代載人飛船 (8,000 kg)", "15000": "空間站核心艙 (15,000 kg)", "35000": "重型貨運補給艙 (35,000 kg)", "60000": "極限超重載荷 (60,000 kg) ⚠️" },
    ...
    en: {
        ...
        configTitle: "🛠️ Mission Configuration", lblEnv: "Launch Lighting:", lblEngine: "Vehicle:", lblPayload: "Payload Mass:",
        lblFuel: "Fuel Load (%):", lblThrottle: "Throttle (%):", lblTurn: "Turn Alt (km):",
        launchBtn: "🔥 Initiate T-10s Terminal Countdown", resetBtn: "🔄 Reset Launch Pad",
        telemetryTitle: "⚙️ Flight Telemetry", ready: "Pad ready. T-10 countdown armed...",
        ...
        gaugeQ: "Dynamic Pressure (Limit: 55 kPa)", gaugeFuel: "Propellant",
        tConfig: "Configuration", tThrust: "Thrust", tOrbit: "Orbit Status",
        tGforce: "Acceleration G-Force", tGmax: "Max Safe: 5.5 G", tIsp: "Specific Impulse",
        tPeri: "Predicted Periapsis", tApo: "Predicted Apoapsis", tEcc: "Eccentricity (e)",
        tSma: "Semi-major Axis", tMoon: "Lunar Distance",
        onPad: "Vehicle on Pad", ascending: "Active Ascent Phase", stableOrbit: "🟢 Stable Orbit Cruise",
        ...
        milestones: { escape: "🚀 T+120s Tower Jettison!", boosters: "⚡ T+160s Stage 1 Separation!", fairing: "✨ T+200s Fairing Separation!", stage2: "🛰️ T+580s Stage 2 Cutoff!", orbit: "🏆 Orbit Complete!" }
    }
};
...
function applyLanguageUI() {
    ...
    setText('lbl-payload', t.lblPayload); setText('lbl-fuel', t.lblFuel); setText('lbl-throttle', t.lblThrottle); setText('lbl-turn', t.lblTurn);
...
function evaluateStructuralLimits(rocket) {
    if (rocket.isDestroyed || !isAdvancedMode) return; // 🟢 簡易模式完全免疫爆炸與結構失效！
    ...
    if (rocket.flightTime > 3.0 && alt < 20 && twr < 1.05) {
        ...
        rocket.failureReason = currentLang === 'zh' ? "推重比不足 (TWR < 1.05)，引擎過熱引爆" : "PAD TWR OVERLOAD: Insufficient thrust.";
    ...
    if (dynQkPa > 55.0 && alt < 20000) {
        rocket.isDestroyed = true;
        rocket.failureReason = currentLang === 'zh' ? `突破 55 kPa 極限 (${dynQkPa.toFixed(1)} kPa)，箭體解體` : `MAX-Q SHEAR FAILURE: Exceeded 55 kPa.`;
    ...
    if (rocket.currentGForce > 5.5 && rocket.flightTime > 10) {
        rocket.isDestroyed = true;
        rocket.failureReason = currentLang === 'zh' ? `過載超過 5.5 G (${rocket.currentGForce.toFixed(2)} G)，結構擠壓破壞` : `G-FORCE OVERLOAD: Exceeded 5.5 G.`;
...
function updateTelemetryValues() {
    ...
    if (machConeMesh) {
        ...
        machConeMesh.material.opacity = isTransonic ? Math.min(0.2, machConeMesh.material.opacity + 0.02) : Math.max(0, machConeMesh.material.opacity - 0.05);
        machConeMesh.material.transparent = true;
        machConeMesh.material.depthWrite = false;
        machConeMesh.material.blending = THREE.AdditiveBlending;
    }
