/**
 * js/rocket_game.js - JarAscent 3D 完整單一自包含核心
 * @license MIT
 */
...
function initRocketScene(containerEl) {
    ...
    scene.add(new THREE.Points(starGeo, new THREE.PointsMaterial({ color: 0xffffff, size: 16, transparent: true, opacity: 0.9 })));
...
function evaluateStructuralLimits(rocket) {
    ...
    if (rocket.flightTime > 3.0 && alt < 20 && twr < 1.05) {
        ...
        rocket.failureReason = currentLang === 'zh' ? "發射台推重比不足 (TWR < 1.05)，引擎過熱引爆" : "PAD TWR OVERLOAD: Insufficient thrust to clear pad.";
    ...
    if (dynQkPa > 55.0 && alt < 20000) {
        rocket.isDestroyed = true;
        rocket.failureReason = currentLang === 'zh' ? `相對動壓突破 55 kPa 極限 (${dynQkPa.toFixed(1)} kPa)，箭體氣動解體` : `MAX-Q SHEAR FAILURE: Exceeded 55 kPa relative pressure.`;
    ...
    if (rocket.currentGForce > 5.5 && rocket.flightTime > 10) {
        rocket.isDestroyed = true;
        rocket.failureReason = currentLang === 'zh' ? `加速度過載超過 5.5 G (${rocket.currentGForce.toFixed(2)} G)，結構被擠壓破壞` : `G-FORCE OVERLOAD: Exceeded 5.5 G.`;
